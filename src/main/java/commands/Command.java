...
public abstract class Command implements OnTriggerListener {
    ...
    public String getString(String key, String... args) {
        String text = TextManager.getString(guildEntity, locale, category.getId(), commandProperties.trigger() + "_" + key, -1, args);
        if (prefix != null) text = text.replace("{PREFIX}", prefix);
        return text;
    }

    public String getString(String key, int option, String... args) {
        String text = TextManager.getString(guildEntity, locale, category.getId(), commandProperties.trigger() + "_" + key, option, args);
        if (prefix != null) text = text.replace("{PREFIX}", prefix);
        return text;
    }

    public String getString(String key, boolean secondOption, String... args) {
        String text = TextManager.getString(guildEntity, locale, category.getId(), commandProperties.trigger() + "_" + key, secondOption ? 1 : 0, args);
        if (prefix != null) text = text.replace("{PREFIX}", prefix);
        return text;
    }
