...
public class SplatnetCommand extends Command implements OnAlertListener {
    ...
    private EmbedBuilder getEmbed(boolean alert) throws ExecutionException, InterruptedException {
        ...
        JSONArray netData = new JSONObject(HttpCache.get(urls[0], Duration.ofMinutes(5)).get().getBody()).getJSONArray("merchandises");
        JSONObject languageData = new JSONObject(HttpCache.get(urls[1], Duration.ofMinutes(5)).get().getBody());
