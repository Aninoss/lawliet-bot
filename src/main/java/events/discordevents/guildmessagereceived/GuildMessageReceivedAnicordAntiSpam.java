...
public class GuildMessageReceivedAnicordAntiSpam extends GuildMessageReceivedAbstract {
    ...
    public boolean onGuildMessageReceived(MessageReceivedEvent event) throws Throwable {
        if (event.getGuild().getIdLong() == AssetIds.ANICORD_SERVER_ID &&
                event.getTextChannel().getIdLong() != 758285721877479504L &&
                event.getTextChannel().getIdLong() != 462405404211675136L &&
                event.getTextChannel().getIdLong() != 693912897998553198L &&
            ...
            if (ratelimitManager.checkAndSet(event.getMember().getIdLong(), 5, Duration.ofSeconds(3)).isPresent()) {
                ...
                event.getGuild().getTextChannelById(462420339364724751L).sendMessage("ANTI RAID (SPAM) FOR " + event.getMember().getUser().getAsTag() + " IN " + event.getTextChannel().getAsMention()).queue();
                return false;
            }
        }
