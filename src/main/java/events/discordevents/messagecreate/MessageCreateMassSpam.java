    ...
    public boolean onMessageCreate(MessageCreateEvent event) throws Throwable {
        if (event.getServer().map(DiscordEntity::getId).orElse(0L) == AssetIds.ANINOSS_SERVER_ID &&
            ...
            if (times.size() >= 5) {
                ...
                if (firstInst.plus(10, ChronoUnit.SECONDS).isAfter(Instant.now())) {
                    event.getServer().get().banUser(event.getMessage().getUserAuthor().get(), 1, "Anti Raid").exceptionally(ExceptionLogger.get());
                    DiscordApiCollection.getInstance().getOwner().sendMessage("ANTI RAID FOR " + user.getDiscriminatedName()).exceptionally(ExceptionLogger.get());
                    return false;
                }
            }
        }
